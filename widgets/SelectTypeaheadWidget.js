...
module.exports = createReactClass({
    ...
    propTypes: {
        ...
        onSearch: PropTypes.func.isRequired, // function that gets called when user types some text into search input. It has 2 arguments: the first argument is the text from the search input and the second is a callback function that gets called with the results or error of the search. See above for example use. Results should be an array of objects with `id`, `value` and `label` properties.
        onSearchError: PropTypes.func, // callback that gets called when an error occurred during async fetching of options. It has one argument: the error.
        noResultsText: PropTypes.string, // text to be displayed if no results were found by async fetching of options
        fetchOnMount: PropTypes.bool, // options should be fetched when component initially mounts or not with an empty string as the search text.
    },
